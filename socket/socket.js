...
const {generateRealMessage,findUserInfoFromDB,removeTempFiles,renameFile,changePicture}=require('../utils/messages');
...
module.exports=(server,Siofu)=>{
    ...
    io.on('connection',(socket)=>{
        ...
        socket.on('join',(params,callback)=>{
            ...
            //empty the user directory anytime he joins a room or just get connected to server
            //let temp_dir=path.join(__dirname,'../public/uploads/jim/temp');
        ...
        socket.on('file_saved',(data,cb)=>{
            ...
            if(data.type=='chat'){
                ...
                const newNamePath=path.join(__dirname,"../public/uploads/"+uploader_username+"/temp/"+fileOriginalname.trim().toLowerCase());
                const newNameRelativePath="/uploads/"+uploader_username+"/temp/"+fileOriginalname.trim().toLowerCase();//this is used by browser to display file later
        ...
        socket.on('disconnect',()=>{
            ...
            if(user){
                ...
                removeTempFiles(user.name)
                    .then(msg=>{
                        if(msg=='ENOENT'){
                            console.log('ENOENT');
                        }else if(msg=='unlink'){
                            console.log('temp files unlinked');
                        }else{
                            console.log('nothing was unlink in temp file');
                        }
                    })
                    .catch(err=>console.log(err));
            }
